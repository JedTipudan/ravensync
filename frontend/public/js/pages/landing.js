    ...
    <div class="min-h-screen" style="background: var(--bg-primary); color: var(--text-primary);">
      ...
      <nav class="fixed top-0 left-0 right-0 z-50 border-b" style="background: var(--glass-bg); backdrop-filter: blur(20px); border-color: var(--border);">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div class="flex items-center justify-between h-16">
            ...
            <div class="hidden md:flex items-center gap-6 text-sm" style="color: var(--text-primary);">
      ...
      <section class="relative min-h-screen flex items-center justify-center pt-16 overflow-hidden animated-bg">
        ...
        <div class="relative z-10 text-center max-w-5xl mx-auto px-4">
          ...
          <h1 class="text-4xl sm:text-7xl font-black mb-6 leading-tight">
            <span class="gradient-text">RavenSync</span><br/>
            <span class="text-3xl sm:text-5xl font-bold" style="color: var(--text-primary);">School Emergency</span><br/>
            <span class="text-2xl sm:text-4xl font-semibold" style="color: var(--text-primary); opacity:0.75;">Communication Platform</span>
          </h1>

          <p class="text-sm sm:text-xl max-w-3xl mx-auto mb-8 leading-relaxed" style="color: var(--text-primary); opacity:0.75;">
          ...
          <div class="flex flex-wrap items-center justify-center gap-3 mb-8">
            <button onclick="navigate('/register')" class="btn btn-primary text-sm sm:text-base px-5 sm:px-8 py-2.5 sm:py-3">
              <i class="fa-solid fa-rocket"></i> Get Started Free
            </button>
            <button onclick="navigate('/login')" class="btn btn-ghost text-sm sm:text-base px-5 sm:px-8 py-2.5 sm:py-3">
              <i class="fa-solid fa-gauge-high"></i> Dashboard
            </button>
            <a href="#features" class="btn btn-ghost text-sm sm:text-base px-5 sm:px-8 py-2.5 sm:py-3">
              <i class="fa-solid fa-play"></i> Learn More
            </a>
          </div>

          <!-- Hero preview card -->
          <div class="rounded-2xl border p-3 sm:p-4 max-w-4xl mx-auto shadow-2xl"
               style="background: var(--card-bg); border-color: var(--border);">
            <div class="flex items-center gap-2 mb-3 px-1">
              <div class="w-2.5 h-2.5 rounded-full bg-red-500"></div>
              <div class="w-2.5 h-2.5 rounded-full bg-yellow-500"></div>
              <div class="w-2.5 h-2.5 rounded-full bg-green-500"></div>
              <span class="text-xs ml-1" style="color: var(--text-primary); opacity:0.6;">RavenSync — Live</span>
              <div class="ml-auto flex items-center gap-1.5">
                <div class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
                <span class="text-xs font-medium text-emerald-500">3 Active Alerts</span>
              </div>
            </div>
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-2 mb-3">
              ${['🚨 Critical', '📢 Broadcast', '👥 2,847', '📡 12 Ch'].map(item => `
                <div class="rounded-xl p-2 text-center" style="background: var(--input-bg);">
                  <div class="text-xs font-semibold" style="color: var(--text-primary);">${item}</div>
                </div>
              `).join('')}
            </div>
            <div class="space-y-1.5">
              ${[
                { color: 'red', title: 'Typhoon Warning — Evacuation', time: '2m ago', sev: 'CRITICAL' },
                ...
                <div class="flex items-center gap-2 rounded-lg px-2 py-1.5" style="background: var(--input-bg);">
                  <div class="w-2 h-2 rounded-full bg-${a.color}-500 animate-pulse flex-shrink-0"></div>
                  <span class="text-xs flex-1 truncate text-left" style="color: var(--text-primary);">${a.title}</span>
                  <span class="text-xs hidden sm:inline" style="color: var(--text-primary); opacity:0.5;">${a.time}</span>
                  <span class="badge badge-${a.sev.toLowerCase()} text-xs">${a.sev}</span>
                </div>
              `).join('')}
            </div>
          </div>
      ...
      <!-- Stats -->
      <section id="stats" class="py-10 sm:py-20 border-y" style="border-color: var(--border);">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-8 text-center">
            ${[
              ...
              <div class="stat-counter" data-target="${stat.value}" data-decimal="${stat.decimal || false}">
                ...
                <div class="text-2xl sm:text-4xl font-black gradient-text counter" id="counter-${i}">0</div>
                <div class="mt-1 text-xs sm:text-base" style="color: var(--text-primary); opacity:0.7;">${stat.label}</div>
              </div>
            `).join('')}
          </div>
        </div>
      ...
      <!-- Features -->
      <section id="features" class="py-12 sm:py-24 max-w-7xl mx-auto px-4">
        <div class="text-center mb-16">
          ...
          <h2 class="text-2xl sm:text-4xl font-bold mb-4" style="color: var(--text-primary);">
            Everything You Need for <span class="gradient-text">Emergency Response</span>
          </h2>
          <p class="max-w-2xl mx-auto" style="color: var(--text-primary); opacity:0.7;">
        ...
        <div class="grid sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
          ${[
            ...
            <div class="rounded-2xl border p-6 card-hover transition-all"
              ...
              <h3 class="text-base sm:text-lg font-bold mb-2" style="color: var(--text-primary);">${f.title}</h3>
              <p class="text-sm leading-relaxed" style="color: var(--text-primary); opacity:0.7;">${f.desc}</p>
            </div>
          `).join('')}
        </div>
      </section>
      ...
      <!-- Why RavenSync -->
      <section id="why" class="py-12 sm:py-24" style="background: var(--bg-secondary);">
        <div class="max-w-7xl mx-auto px-4">
          <div class="text-center mb-16">
            <h2 class="text-2xl sm:text-4xl font-bold mb-4" style="color: var(--text-primary);">
              ...
              ${[
                ...
                <div class="flex gap-4">
                  ...
                  <div>
                    ...
                    <p class="text-sm" style="color: var(--text-primary); opacity:0.7;">${item.desc}</p>
                  </div>
                </div>
              `).join('')}
            </div>
      ...
      <!-- Testimonials -->
      <section class="py-12 sm:py-24 max-w-7xl mx-auto px-4">
        <div class="text-center mb-16">
          <h2 class="text-2xl sm:text-4xl font-bold mb-4" style="color: var(--text-primary);">
            ...
            <div class="rounded-2xl p-6 border card-hover" style="background: var(--card-bg); border-color: var(--border);">
              <div class="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <div class="text-xs" style="color: var(--text-primary); opacity:0.6;">${t.org}</div>
                </div>
              </div>
              <p class="text-sm leading-relaxed italic" style="color: var(--text-primary); opacity:0.75;">"${t.quote}"</p>
      ...
      <!-- Team -->
      <section id="team" class="py-12 sm:py-24" style="background: var(--bg-secondary);">
        <div class="max-w-7xl mx-auto px-4">
          <div class="text-center mb-16">
            <h2 class="text-2xl sm:text-4xl font-bold mb-4" style="color: var(--text-primary);">
              Meet the <span class="gradient-text">Team</span>
            </h2>
            <p style="color: var(--text-primary); opacity:0.7;">The engineers behind RavenSync's emergency communication platform.</p>
          </div>
          <div class="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3 sm:gap-6">
            ${[
              ...
              <div class="rounded-2xl p-6 text-center border card-hover" style="background: var(--card-bg); border-color: var(--border);">
                ...
                <div class="font-bold text-xs sm:text-sm mb-1" style="color: var(--text-primary);">${m.name}</div>
                <div class="text-xs" style="color: var(--text-primary); opacity:0.6;">${m.role}</div>
              </div>
            `).join('')}
          </div>
        </div>
      ...
      <!-- FAQ -->
      <section id="faq" class="py-12 sm:py-24 max-w-4xl mx-auto px-4">
        <div class="text-center mb-16">
          <h2 class="text-2xl sm:text-4xl font-bold mb-4" style="color: var(--text-primary);">
        ...
        <div class="space-y-4" id="faq-list">
          ${[
            ...
            <div class="rounded-xl border overflow-hidden" style="background: var(--card-bg); border-color: var(--border);">
              ...
              <div id="faq-answer-${i}" class="hidden px-4 sm:px-6 pb-4 text-sm leading-relaxed" style="color: var(--text-primary); opacity:0.75;">${item.a}</div>
            </div>
          `).join('')}
        </div>
      </section>
      ...
      <section class="py-12 sm:py-24">
        <div class="max-w-4xl mx-auto px-4 text-center">
          <div class="rounded-3xl border p-6 sm:p-12 relative overflow-hidden"
            ...
            <div class="relative z-10">
              ...
              <p class="mb-6 sm:mb-8 max-w-xl mx-auto" style="color: var(--text-primary); opacity:0.75;">
                Join thousands of organizations using RavenSync for real-time emergency communication and disaster response.
              </p>
              <div class="flex flex-wrap gap-4 justify-center">
                <button onclick="navigate('/register')" class="btn btn-primary text-sm sm:text-base px-6 sm:px-10 py-2.5 sm:py-3">
                  <i class="fa-solid fa-rocket"></i> Start for Free
                </button>
                <button onclick="navigate('/login')" class="btn btn-ghost text-sm sm:text-base px-6 sm:px-10 py-2.5 sm:py-3">
      ...
      <!-- Footer -->
      <footer class="border-t py-8 sm:py-12" style="border-color: var(--border);">
        <div class="max-w-7xl mx-auto px-4">
          <div class="grid grid-cols-2 md:grid-cols-4 gap-6 sm:gap-8 mb-8">
            <div>
              ...
              <p class="text-sm" style="color: var(--text-primary); opacity:0.7;">Real-time emergency communication platform for modern communities.</p>
            </div>
            ${[
              ...
              <div>
                ...
                <ul class="space-y-2">
                  ${col.links.map(l => `<li><a href="#" class="text-sm transition-colors hover:text-indigo-500" style="color: var(--text-primary); opacity:0.65;">${l}</a></li>`).join('')}
                </ul>
              </div>
            `).join('')}
          </div>
          <div class="border-t pt-8 flex flex-col md:flex-row items-center justify-between gap-4" style="border-color: var(--border);">
            <p class="text-sm" style="color: var(--text-primary); opacity:0.6;">© 2025 RavenSync. Enterprise Emergency Communication Platform.</p>
            <div class="flex items-center gap-2 text-xs" style="color: var(--text-primary); opacity:0.6;">
              <div class="w-2 h-2 rounded-full bg-emerald-500 animate-pulse"></div>
